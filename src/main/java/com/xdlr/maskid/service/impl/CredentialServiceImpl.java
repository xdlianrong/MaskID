...
import com.xdlr.maskid.constant.CredentialFieldDisclosureValue;
import com.xdlr.maskid.constant.ErrorCode;
import com.xdlr.maskid.constant.ParamKeyConstant;
import com.xdlr.maskid.exception.maskidBaseException;
...
import com.xdlr.maskid.service.BaseService;
import com.xdlr.maskid.util.CredentialUtils;
import com.xdlr.maskid.util.DataToolUtils;
import com.xdlr.maskid.util.DateUtils;
...
public class CredentialServiceImpl extends BaseService implements CredentialService {
    ...
    public ResponseData<CredentialWrapper> createCredential(CreateCredentialArgs args) {
        ...
        try {
            ...
            Map<String, String> credentialProof = CredentialUtils.buildCredentialProof(
                    result,
                    args.getmaskidPrivateKey().getPrivateKey(),
                    disclosureMap);
     ...
     * @param maskidPrivateKey the passed-in privateKey and maskid bundle to sign
     * @return the modified CredentialWrapper
     */
    @Override
    public ResponseData<Credential> addSignature(
            List<Credential> credentialList,
            maskidPrivateKey maskidPrivateKey) {
        if (credentialList == null || credentialList.size() == 0 || !maskidUtils
                .isPrivateKeyValid(maskidPrivateKey)) {
        ...
        String privateKey = maskidPrivateKey.getPrivateKey();
        ECKeyPair keyPair = ECKeyPair.create(new BigInteger(privateKey));
        String keymaskid = maskidUtils
                .convertAddressTomaskid(new Address(Keys.getAddress(keyPair)).toString());
        if (!maskidService.ismaskidExist(keymaskid).getResult()) {
            return new ResponseData<>(null, ErrorCode.maskid_DOES_NOT_EXIST);
        }
        result.setIssuer(keymaskid);
    ...
    private ResponseData<Boolean> verifyIssuerExistence(String issuermaskid) {
        ResponseData<Boolean> responseData = maskidService.ismaskidExist(issuermaskid);
    ...
    private ResponseData<Boolean> verifySignature(
        ...
        try {
            ...
            if (StringUtils.isEmpty(publicKey)) {
                ...
                ResponseData<maskidDocument> innerResponseData =
                        maskidService.getmaskidDocument(credentialIssuer);
                if (innerResponseData.getErrorCode() != ErrorCode.SUCCESS.getCode()) {
                    logger.error(
                            "Error occurred when fetching maskid document for: {}, msg: {}",
                            credentialIssuer, innerResponseData.getErrorMessage());
                    return new ResponseData<>(false, ErrorCode.CREDENTIAL_maskid_DOCUMENT_ILLEGAL);
                } else {
                    maskidDocument maskidDocument = innerResponseData.getResult();
                    ErrorCode errorCode = DataToolUtils
                            .verifySecp256k1SignatureFrommaskid(rawData, credential.getSignature(),
                                    maskidDocument);
                    if (errorCode.getCode() != ErrorCode.SUCCESS.getCode()) {
                        errorCode = DataToolUtils
                                .verifySignatureFrommaskid(rawData, signatureData, maskidDocument);
                        if (errorCode.getCode() != ErrorCode.SUCCESS.getCode()) {
                            return new ResponseData<>(false, errorCode);
                        }
                    }
